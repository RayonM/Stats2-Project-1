...
p1 = attributes %>%
  ggplot(aes(x = Driven_Wheels, y = logMSRP)) +
  geom_boxplot()
...
p3 = attributes %>%
  ggplot(aes(x = Transmission.Type, y = logMSRP)) +
  geom_boxplot()

p4 = attributes %>%
  ggplot(aes(x = Vehicle.Size, y = logMSRP)) +
  geom_boxplot()
...
train_ = subset(train, select = c("Engine.HP", "Number.of.Doors", "city.mpg", "Popularity", "MSRP"))
train_$city.mpg = log(train_$city.mpg)
train_MSRP = log(train_$MSRP)
test_ = subset(test, select = c("Engine.HP", "Number.of.Doors", "city.mpg", "Popularity", "MSRP"))
test_$city.mpg = log(test_$city.mpg)
test_MSRP = log(test_$MSRP)
validation_ = subset(validate, select = c("Engine.HP", "Number.of.Doors", "city.mpg", "Popularity", "MSRP"))
...
knn_model = train(MSRP~., data = train_, method = "knn", preProcess = c("center", "scale"), trControl = fitControl, tuneGrid = data.frame(l = c(1:10, 15, 20, 25, 30)))
knn_model
...
# Making Predicitons on the validation set
...
validation_ase_knn = mean(train_$MSRP - predict(knn_model, validation_))^2

validation_ase_knn

plot(knn_pred_validation, validation_$MSRP)
lines(0:2000, 0:2000)

varImp(knn_model)
plot(varImp(knn_model), main = "Importance of Varibales for the KNN Model")
...
fitControl = trainControl(method = "repeatedcv", number = 10, repeats = 10)
reg_tree_model = train(MSRP~., data = train_, method = "rpart", minsplit = 5, trControl = fitcontrol,
                       tuneGrid = data.frame(cp=c(.005,.0008,.01,.015,.02,.025,.03,.035,.04,.05,.06,.07.08.09.,.25,.04)))
...
test_ase_ref_tree = mean((train_$MSRP - predict(reg_tree_model, test_))^2)
...
reg_tree_validate = postResample(pred = reg_tree_pred, obs = validation_$MSRP)
reg_tree_validate

validation_ase_reg_tree = mean((train_$MSRP - predict(reg_tree_model, validation_))^2)

validation_ase_reg_tree

plot(reg_tree_pred, validation_$MSRP)
lines(0:2000, 0:2000)

varImp(reg_tree_model)

plot(varImp(reg_tree_model), main = "Importaance of Variable of Regressopm Tree Model")
```